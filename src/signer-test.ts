...
export class TestGcpKmsSigner extends AbstractSigner {
  ...
  constructor(kmsCredentials: GcpKmsSignerCredentials, provider?: Provider, privateKey?: string) {
    super(provider);
    defineProperties<TestGcpKmsSigner>(this, { provider: provider || null });
    defineProperties<TestGcpKmsSigner>(this, { kmsCredentials });
  ...
  async getAddress(): Promise<string> {
    if (this.ethereumAddress === undefined) {
      if (this.privateKey) {
        this.ethereumAddress = new Wallet(this.privateKey).address;
      } else {
  ...
  async _signDigest(digestString: string): Promise<string> {
    ...
    if (this.privateKey) {
      ...
      sig = await requestKmsSignature(digestBuffer, this.kmsCredentials);
    }
